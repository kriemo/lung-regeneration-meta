...
if(!file.exists(so_fn)){
  load(file.path(data_dir, study_id, "data",
                "EpiHiRes_seurat.RData"))
  so <- UpdateSeuratObject(subset)
  #13 = AT1?
  #4 and 9 = TD
  #3 = naive ATII(d14_PBS)
  new_ids <- c(
    "9" = "Krt8_c9-timecourse",
    "4" = "Krt8_c4-timecourse",
    "13" = "AT1-timecourse",
    "3" = "ATII-timecourse"
  )
  so$cell_type <- ifelse(so$res.1 %in% names(new_ids),
                         new_ids[so$res.1],
                         "other")
  ...
  so_subset <- UpdateSeuratObject(seu)
  so_subset$cell_type <- case_when(
    so_subset$cell_type == "AT1 Cells" ~ "AT1-whole-lung",
    so_subset$cell_type == "AT2 after Injury" ~ "AT2-injured-whole-lung",
    so_subset$cell_type == "AT2 Cells" ~ "AT2-whole-lung",
    so_subset$cell_type == "Regenerating Epithelium" ~ "Krt8-whole-lung"
    )
  # saveRDS(so_subset, file.path(data_dir, study_id, "so_subset.rds"))
  so <- merge(so, so_subset)
...
colnames(avg_expr)[2:3] <- paste0(study_id, "::", colnames(avg_expr)[2:3], "_d21")
avg_expr <- rownames_to_column(avg_expr, "mouse_gene_name")
...
fit <- euler(combined_markers[1:3])
...
upset(fromList(combined_markers[4:7]))
