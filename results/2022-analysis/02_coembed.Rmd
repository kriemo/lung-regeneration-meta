...
if(!file.exists(sce_fn)){
  ...
  sce <- readRDS(sce_fn)
  metadata(sce)$integration <- "fastMNN"
}
...
if(!file.exists(sce_fn)){
  ...
  sce <- readRDS(sce_fn)
  metadata(sce)$integration <- "harmony"
 }
...
library(purrr)
library(stringr)
data.frame(original_cell_type = paste0(str_split(sce$study, "_") |> map_chr(1), 
                                       ": ", 
                                       sce$cell_type), 
