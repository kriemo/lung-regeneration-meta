...
if(!file.exists(sce_fn)){
  ...
  mnn_sce <- multiBatchNorm(mnn_sce, batch = mnn_sce$study, preserve.single = TRUE)
  
  snn.gr <- buildSNNGraph(mnn_sce, use.dimred="corrected")
  mnn_clusters <- igraph::cluster_walktrap(snn.gr)$membership
  mnn_sce$mnn_clusters <- as.factor(mnn_clusters)
  
  mnn_tab <- table(Cluster = mnn_clusters, Batch = mnn_sce$study)
...
if(!file.exists(sce_fn)){
  ...
  hm_clusters <- igraph::cluster_walktrap(snn.gr)$membership
  hm_sce$hm_clusters <- as.factor(hm_clusters)
  hm_tab <- table(Cluster = hm_clusters, Batch = hm_sce$study)
...
if(!file.exists(sce_fn)){
 # source("scVI.R")
  ...
  if(!"scvi_clusters" %in% colnames(colData(scvi_sce))){
    snn.gr <- buildSNNGraph(scvi_sce, use.dimred="scvi")
    scvi_clusters <- igraph::cluster_walktrap(snn.gr)$membership
    scvi_sce$scvi_clusters <- as.factor(scvi_clusters)
    scvi_tab <- table(Cluster = scvi_clusters, Batch = scvi_sce$study)
...
if(!file.exists(sce_fn)){
 # source("scVI.R")
  ...
  if(!"scanvi_clusters" %in% colnames(colData(scanvi_sce))){
    snn.gr <- buildSNNGraph(scanvi_sce, use.dimred="scanvi")
    scanvi_clusters <- igraph::cluster_walktrap(snn.gr)$membership
    scanvi_sce$scanvi_clusters <- as.factor(scanvi_clusters)
    scanvi_tab <- table(Cluster = scanvi_clusters, Batch = scanvi_sce$study)
